...
public class FmController {
    ...
    public String article(@PathVariable String cid, Map<String, Object> map) {
        ...
        article.put("dateModified", RmUtil.transDate(content.getModified().toString()));
    ...
    @RequestMapping(value = "/fm/comment/{cid}", method = RequestMethod.POST)
    public Result comment(@RequestBody Comment comment, @PathVariable Long cid) {
        comment.setCid(cid);// 文章内容ID
        comment.setCreated(new Date().getTime()/1000);
        comment.setIp(RmUtil.getClientIPAdrress(request));
        comment.setAgent(request.getHeader("User-Agent"));
        comment.setType("comment");
        comment.setStatus("approved");
        comment.setParent(0L);
        comment.setOwnerId(1L);
